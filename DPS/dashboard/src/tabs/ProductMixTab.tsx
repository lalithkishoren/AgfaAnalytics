...
export const ProductMixTab: React.FC<Props> = ({ filters }) => {
  // Filtered data
  const filteredBCs = amspByBudgetClass
    .filter(d => filters.bu === 'All' || d.bu === filters.bu)
    .filter(d => filters.budgetClass === 'All' || d.name === filters.budgetClass);

  const filteredScatterData = productMixData.filter(d =>
    d.bu !== 'M0' &&   // M0 aggregate excluded from scatter
    (filters.bu === 'All' || d.bu === filters.bu)
  );

  const filteredCopaByBU = copaByBU.filter(d =>
    filters.bu === 'All' || d.bu === filters.bu
  );

  // CO-PA chart data mapped from filtered set
  const copaChartData = filteredCopaByBU.map((d) => ({
    ...d,
          ...
          <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', height: '100%' }}>
            ...
            <TableContainer>
              <Table size="small">
                ...
                <TableBody>
                  {[...filteredBCs].sort((a, b) => b.margin - a.margin).map((row, i) => (
                    ...
                    <TableRow>
                      <TableCell colSpan={4} align="center">
                        <Typography variant="caption" color="text.secondary">
                          No budget classes match the current filter.
                        </Typography>
                      </TableCell>
                    </TableRow>
                  )}
                  ...
                  <Bar dataKey="netTOMillion" name="Net TO (€M)" radius={[0, 4, 4, 0]}>
                    ...
                    <LabelList
                      ...
                      formatter={(v: string | number | undefined) => {
                        ...
                        const match = filteredCopaByBU.find(d => d.netTO === num);
                        return `€${num.toFixed(1)}M (${match?.pct ?? ''}%)`;
                      }}
                    ...
                    <TableRow sx={{ bgcolor: '#F5F5F5' }}>
                      ...
                      <TableCell align="right">
                        <Typography variant="caption" sx={{ fontFamily: 'monospace', fontWeight: 700 }}>
                          €{filteredCopaByBU.reduce((s, r) => s + r.netTO, 0).toFixed(1)}M
                        </Typography>
                      </TableCell>
                      <TableCell align="right">
                        <Typography variant="caption" sx={{ fontWeight: 700 }}>
                          {filters.bu === 'All' ? '100%' : `${filteredCopaByBU.reduce((s, r) => s + r.pct, 0).toFixed(1)}%`}
                        </Typography>
                      </TableCell>
                    </TableRow>
                  </TableBody>
