...
export const GeographicTab: React.FC<Props> = ({ filters }) => {
  // Apply region filter
  const filteredGeoData = geographicData.filter(d =>
    filters.region === 'All' || d.region === filters.region
  );

  const filteredRegionalSummary = regionalSummary.filter(d =>
    filters.region === 'All' || d.region === filters.region
  );
  ...
  // Top 10 sales orgs by FY2025 OIT — drawn from the filtered set
  const top10SalesOrgs = [...filteredSalesOrg]
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0' }}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={filteredRegionalSummary} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                <Bar dataKey="netSales" name="Net Sales" radius={[4, 4, 0, 0]}>
                  {filteredRegionalSummary.map((entry) => (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                ...
                <Legend
                  ...
                  formatter={(value) => {
                    const reg = filteredRegionalSummary.find(r => r.region === value);
            ...
            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5 }}>
              ...
              {filters.region !== 'All' && (
                <Chip
                  ...
                  size="small"
                  sx={{ ml: 1, bgcolor: `${REGION_COLORS[filters.region] || '#9E9E9E'}20`, color: REGION_COLORS[filters.region] || '#9E9E9E', border: `1px solid ${REGION_COLORS[filters.region] || '#9E9E9E'}60`, height: 18, fontSize: '0.65rem' }}
                />
              )}
            </Typography>
            <TableContainer>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} align="center">
                        <Typography variant="caption" color="text.secondary">
                          No countries match the current region filter.
                        </Typography>
                      </TableCell>
                    </TableRow>
                  )}
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', mb: 2 }}>
          ...
          <ChartNote
            ...
            note={`Top 10 sales orgs by FY2025 OIT shown${filters.region !== 'All' ? ` (filtered to ${filters.region})` : '. 58 total DPS sales organizations'}.`}
          />
        </Paper>
        ...
        <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0' }}>
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1.5, fontStyle: 'italic' }}>
            {filters.region !== 'All'
              ? `Showing sales organizations in ${filters.region}. OIT unit counts from Order follow-up file.`
