...
export const OrderPipelineTab: React.FC<Props> = ({ filters }) => {
  ...
  // Filter oitBySalesOrg by region
  const filteredSalesOrg = oitBySalesOrg.filter(d =>
    filters.region === 'All' || d.region === filters.region
  );

  // Active filter label
  const activeFilterLabel = [
    filters.year !== 'All' && `Year: FY${filters.year}`,
    filters.bu !== 'All' && `BU: ${filters.bu}`,
    filters.budgetClass !== 'All' && `BC: ${filters.budgetClass}`,
    filters.region !== 'All' && `Region: ${filters.region}`,
  ].filter(Boolean).join(' · ');

  // Determine which bars to render in the Monthly OIT FY2024 vs FY2025 chart
  const showFY2024Bar = filters.year === 'All' || filters.year === '2024';
  const showFY2025Bar = filters.year === 'All' || filters.year === '2025';
  const show2026Note = filters.year === '2026';

  // Highlight border for FY2026 sections when year filter is 2026
  const fy2026HighlightSx = filters.year === '2026'
    ? { border: '2px solid #1565C0', boxShadow: '0 0 0 3px #1565C020' }
    : {};
    ...
    <Box>
      ...
      {activeFilterLabel && (
        <Alert severity="info" sx={{ mb: 2, fontSize: '0.82rem' }}>
          <strong>Filter active:</strong> {activeFilterLabel} — Applicable charts filtered. KPI totals reflect full-scope data.
        </Alert>
      )}
      ...
      ═══════════════════════════════════════════════════════════════ */}
      <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', mb: 3, ...fy2026HighlightSx }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
          ...
          {filters.year === '2026' && (
            <Chip
              label="FY2026 Selected"
              size="small"
              sx={{ bgcolor: '#1565C0', color: '#fff', height: 18, fontSize: '0.65rem' }}
            />
          )}
        </Box>
      ...
      <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', mb: 3, ...fy2026HighlightSx }}>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1.5 }}>
          Total: 24 units (Jan+Feb confirmed + Mar MTD). Anapurna/Accurio dominant at 54%. Note: equipment family data has no BU/region attribute — shown as-is regardless of filter.
        </Typography>
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                ...
                <Tooltip
                  formatter={(value: number | undefined, name: string) => [`${value ?? 0} units`, name]}
                />
                <Legend
          ...
          <Grid size={{ xs: 12, md: 7 }}>
            <ResponsiveContainer width="100%" height={220}>
              ...
              >
                ...
                <Tooltip formatter={(value: number | undefined) => [`${value ?? 0} units`, 'OIT Q1 2026']} />
      ...
      <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', mb: 3, ...fy2026HighlightSx }}>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <AreaChart data={backlog2026} margin={{ top: 10, right: 30, left: 5, bottom: 5 }}>
            ...
            <Tooltip
              labelStyle={{ fontWeight: 700 }}
              formatter={(value: number | undefined) => [`${value ?? 0} units`, 'Backlog']}
            />
            <Area
              type="monotone"
              dataKey="units"
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1.5 }}>
          ...
          {filters.region !== 'All' && ` Filtered to region: ${filters.region}.`}
        </Typography>
              ...
              {filteredSalesOrg.length === 0 && (
                <TableRow>
                  <TableCell colSpan={5} align="center">
                    <Typography variant="caption" color="text.secondary">
                      No sales organizations match the current region filter.
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
          ...
          <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0' }}>
            ...
            {show2026Note ? (
              <Alert severity="info" sx={{ fontSize: '0.82rem' }}>
                Year 2026 selected — see FY2026 sections above for current-year OIT and pipeline data.
              </Alert>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
