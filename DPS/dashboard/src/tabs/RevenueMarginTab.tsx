...
export const RevenueMarginTab: React.FC<Props> = ({ filters }) => {
  ...
  const filteredFamilies = topProductFamilies.filter(d =>
    (filters.bu === 'All' || d.buCode === filters.bu) && d.family !== 'OTHER'
  );
  ...
  const topFamiliesFiltered = filteredFamilies.slice(0, 9);

  return (
    <Box>
      {activeFilterLabel && (
        <Alert severity="info" sx={{ mb: 2, fontSize: '0.82rem' }}>
          <strong>Filter active:</strong> {activeFilterLabel} — Charts and tables showing filtered data where applicable. KPI totals reflect full-scope data.
        </Alert>
      )}
          ...
          <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', height: '100%' }}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={monthlyRevenueTrend} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                />
                <Tooltip
                  formatter={(value: number | undefined) => [`€${((value ?? 0) / 1000000).toFixed(1)}M`, 'Net Sales']}
          ...
          <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0', height: '100%' }}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <ComposedChart data={monthlyRevenueTrend} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                />
                <Tooltip
                  formatter={(value: number | undefined) => [`${value ?? ''}%`, 'AMSP Margin']}
                  labelStyle={{ fontWeight: 700 }}
                />
                <Line
                  type="monotone"
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Paper elevation={0} sx={{ p: 2, border: '1px solid #e0e0e0' }}>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <BarChart
                data={[...filteredBCs].sort((a, b) => b.margin - a.margin)}
                layout="vertical"
              ...
              >
                ...
                <Bar dataKey="margin" name="AMSP Margin %" radius={[0, 4, 4, 0]}>
                  {[...filteredBCs].sort((a, b) => b.margin - a.margin).map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
