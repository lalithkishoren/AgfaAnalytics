...
export const dataGaps = [
  ...
  {
    kpi: 'No Join Key: OIT → Revenue Recognition → Revenue & Margin',
    reason: 'No shared SAP order number or CRM ID exists to link the unit order tracker (manual Excel) with financial reporting (RECO/AMSP/CO-PA in SAP BW). OIT is tracked in a manually maintained Excel file; revenue and margin are extracted from SAP BW. There is no common key field between them.',
    impact: 'Cannot compute revenue per order, margin per printer sold, or OIT-to-RR conversion rate in EUR. The order lifecycle — from intake through revenue recognition to margin — cannot be traced end-to-end.',
    recommendation: 'Enforce SAP Sales Order (SD) number in the OIT follow-up file at booking time. Alternatively, integrate with CRM (Salesforce/equivalent) that captures both unit pipeline and financial outcomes under a single opportunity ID.',
  },
];
